...
flags = tf.flags
flags.DEFINE_string('device', '0', '显卡')
flags.DEFINE_string('config', './cfg0.yml', '配置文件')
...
class AVGRunner:
    ...
    def run_benchmark(self, iter, mode="Test"):
        ...
        while not test_iter.use_up:
            ...
            in_data = np.zeros(shape=(self._batch, self._in_seq, 900, 900, c.IN_CHANEL))
            gt_data = np.zeros(shape=(self._batch, self._out_seq, 900, 900, 1))
            if type(data) == type([]):
                break
            print(data.shape)
            in_data[...] = data[:, :self._in_seq, :, :, :]
...
if __name__ == '__main__':
    ...
    test('/extend/gru_tf_data/5l_stlstm_incept_ebgan/Save/model.ckpt-199999','199999-2029')
